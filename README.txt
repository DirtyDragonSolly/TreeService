чтобы поднять всё в контейнерах нужно в папке где находится docker-compose.yml выполнить команду docker compose up -d --force-recreate --no-deps --build и после этого можно открыть http://127.0.0.1:5280/index.html
